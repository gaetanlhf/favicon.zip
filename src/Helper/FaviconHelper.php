...
class FaviconHelper
{
    ...
    public function generate(string $path, string $id, string $appName, string $shortAppName, string $language, string $startUrl, string $themeColour, string $backgoundColour, string $display, bool $sixtyFourIco, bool $fortyEightIco, bool $apple, bool $android, bool $ms): Response
    {
        $config = array_merge([
        ...
        try {
            ...
            if ($ms) {
                ...
                $file = str_replace("/favicon", "", $file);
                $this->filesystem->remove($path . $id . "/favicon/browserconfig.json");
                $this->filesystem->appendToFile($path . $id . "/favicon/browserconfig.json", $file);
            }
